...
3. 在右上角选择版式模板；模板会改变分栏、分段和展示形式，不会替换正文内容。
4. 点击简历里的文字继续修改，并用右上角的字号控制调整整体字号。
5. 如果只想导出一页，右上角选择 `单页 A4`。
6. 点击 `导出 PDF`，在浏览器打印窗口里保存为 PDF。
...
4. 右上角常用按钮：
   ...
   - `模板`：在 `经典单栏`、`左右分栏`、`商务模块`、`分析师时间轴` 之间切换版式
   - `PDF 长度`：选择 `单页 A4` 或 `自动分页`
   - `字号`：调整整份简历的字号大小
   - `恢复内容`：清空当前正文修改，恢复默认内容；不会改变已选择的视觉模板
...
10. 右上角选择合适模板和 `单页 A4`，调整字号后点击 `导出 PDF`。打印窗口选择 A4、100% 缩放、关闭页眉页脚，并建议开启背景图形。
